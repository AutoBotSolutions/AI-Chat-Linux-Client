"""
OpenRouter client for multi-model routing abstraction.
"""
...
class OpenRouterClient(BaseAPIClient):
    ...
    def __init__(self, api_key: str, base_url: str = "https://openrouter.ai/api/v1"):
        ...
        self.headers = {
            ...
            "HTTP-Referer": "https://chat-linux-client.local",
            "X-Title": "Chat Linux Client"
        }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/models", headers=self.headers) as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        self.logger.error(f"OpenRouter list models error: {response.status} - {error_text}")
        except Exception as e:
            self.logger.error(f"OpenRouter list models failed: {e}")
        return []
    ...
    async def chat_completion(
        ...
        if not self.is_available:
            yield "Error: OpenRouter provider not available"
            return
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        self.logger.error(f"OpenRouter API error: {response.status} - {error_msg}")
                        yield f"Error: OpenRouter API returned {response.status}: {error_msg}"
